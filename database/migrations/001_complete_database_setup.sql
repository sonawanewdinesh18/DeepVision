-- ============================================================
-- DEEPVISION COMPLETE DATABASE SETUP
...
-- Profiles table with is_active column
...
BEGIN
    ...
    INSERT INTO public.profiles (id, email, full_name, role, subscription_plan, is_active)
    VALUES (
        NEW.id,
...
DROP POLICY IF EXISTS "payment_history_policy" ON public.payment_history;
...
INSERT INTO public.subscription_plans (name, price, currency, interval, features, max_detections)
VALUES
    ('Free', 0, 'USD', 'month', '["50 detections/month"]'::jsonb, 50),
    ('Pro', 999, 'INR', 'month', '["Unlimited detections"]'::jsonb, NULL),
    ('Enterprise', 0, 'USD', 'year', '["Unlimited detections", "API access"]'::jsonb, NULL)
...
SELECT 'DeepVision database setup complete! All tables, policies, and triggers created successfully.' as message;