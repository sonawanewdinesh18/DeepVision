"""
app/api/v1/user.py
User profile, settings, analytics, subscription, and payment endpoints.
"""
...
async def get_user_subscription(
    ...
    if stats_response.data:
        detections_used = stats_response.data[0].get("total_detections", 0)
    ...
    return UserSubscriptionResponse(
        plan=plan.get("name", "free").lower(),
        status=subscription.get("status", "active"),
        current_period_start=subscription.get("current_period_start"),
        current_period_end=subscription.get("current_period_end"),
        cancel_at_period_end=subscription.get("cancel_at_period_end", False),
        detections_used=detections_used,
        detections_limit=plan.get("max_detections")
    )
...
async def get_subscription_plans(
    ...
    response = supabase.table("subscription_plans").select("*").eq(
        "is_active", True
    ...
    for plan in (response.data or []):
        plans.append(SubscriptionPlanResponse(
            ...
            max_detections=plan.get("max_detections"),
            is_active=plan.get("is_active", True),
            is_current=(plan["name"].lower() == current_user.subscription_plan.lower())
        ))
    
    return plans


@router.post("/subscription/upgrade", response_model=SuccessResponse)
async def upgrade_subscription(
    ...
    supabase.table("payment_history").insert({
        ...
        "transaction_id": f"TXN{datetime.utcnow().timestamp():.0f}"
    }).execute()
...
@router.post("/subscription/cancel", response_model=SuccessResponse)
async def cancel_subscription(
    ...
    return SuccessResponse(
        ...
        data={"cancel_at": subscription.get("current_period_end")}
    )
...
async def get_payment_history(
    ...
    # Get total count
    count_response = supabase.table("payment_history").select(
        "id", count="exact"
    ).eq("user_id", current_user.id).execute()
    ...
    for payment in (response.data or []):
        plan_name = None
        if payment.get("subscriptions") and payment["subscriptions"].get("subscription_plans"):
            plan_name = payment["subscriptions"]["subscription_plans"].get("name")
...
async def get_payment_details(
    ...
    response = supabase.table("payment_history").select(
        "*, subscriptions(subscription_plans(name))"
    ).eq("id", payment_id).eq("user_id", current_user.id).execute()
    ...
    return PaymentDetails(
        ...
        transaction_id=payment.get("transaction_id"),
        subscription_plan=plan_name,
        invoice_url=None,  # TODO: Generate invoice URL
        created_at=payment["created_at"]
    )
